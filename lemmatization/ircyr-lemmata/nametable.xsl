...
<xsl:key name="regindex" match="name" use="concat(translate(normalize-space(.), ' ', ''),@nymRef)" />
	...
	<body>		
		<xsl:for-each select="//div[@type='edition']//name[generate-id(.)=generate-id(key('index', translate(normalize-space(.), ' ', ''))[1])]">
			...
			<name>
				...
				<allregs>
					<xsl:for-each select="//div[@type='edition']//name[translate(normalize-space(.), ' ', '')=translate(normalize-space(current()), ' ', '')][generate-id(.)=generate-id(key('regindex', concat(translate(normalize-space(.), ' ', ''),@nymRef))[1])]">
						<xsl:value-of select="@nymRef" />
						<xsl:text> </xsl:text>
					</xsl:for-each>
				</allregs>
